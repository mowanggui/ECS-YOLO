...
sys.path.append('/home/algointern/project/EMS-YOLO-main/utils/loggerss')
sys.path.append('/home/algointern/project/EMS-YOLO-main/utils/loggerss/wandb')
...
import val3 as validate  # for end-of-epoch mAP
...
from comet.comet_utils import check_comet_resume
from loss_tal import v10DetectLoss
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    compute_loss = v10DetectLoss(model)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            # log_vals = {**label_loss_items(mloss) + results + lr}
            # print(len(list(mloss)))
            # print(len(list(results)))
            # print(len(lr))
            # print(len(log_vals))
            callbacks.run('on_fit_epoch_end', log_vals, epoch, best_fitness, fi, 10)
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    ...
                    if is_coco:
                        callbacks.run('on_fit_epoch_end', list(mloss) + list(results) + lr, epoch, best_fitness, fi, 10)
...
def parse_opt(known=False):
    ...
    parser.add_argument('--cfg', type=str, default='res10v10.yaml', help='model.yaml path')
