...
def create_dataloader(path, sample_size, T, image_shape, mode, batch_size, stride, single_cls=False, hyp=None,
                      augment=False, cache=False, pad=0.0,
    ...
    sampler = None if rank == -1 else distributed.DistributedSampler(dataset,
                                                                     shuffle=shuffle)  # 定义取batch的方法，是一个迭代器， 每次生成一个key 用于读取dataset中的值
