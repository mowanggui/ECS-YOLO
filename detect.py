...
from dataloader import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams
from general import (LOGGER, check_file, check_img_size, check_imshow, check_requirements, colorstr,
...
def run(weights=ROOT / 'yolov3.pt',  # model.pt path(s) 事先训练完成的权重文件，比如yolov5s.pt,默认 weights/，假如使用官方训练好的文件（比如yolov5s）,则会自动下载
        source=ROOT / 'data/images',  # file/dir/URL/glob, 0 for webcam
        imgsz=640,  # inference size (pixels) 预测时的放缩后图片大小
        ...
        save_crop=False,  # save cropped prediction boxes 是否保存裁剪后的预测框，默认为False
        nosave=False,  # do not save images/videos 不保存图片、视频
        ...
        hide_conf=False,  # hide confidences 若为True: 隐藏标签
        half=False,  # use FP16 half-precision inference 若为True: 隐藏标签
    ...
    # 判断source是否是链接
    # .lower()转化成小写 .upper()转化成大写 .title()首字符转化成大写，其余为小写, .startswith('http://')返回True or Flase0
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # ===以下部分是做预处理===#
        t1 = time_sync() # 获取当前时间
        im = torch.from_numpy(im).to(device) # 将图片放到指定设备(如GPU)上识别。#shape:[3,480,640]
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default='D:/PycharmProjects/EMS-YOLO-main/bdd100k/images/test', help='file/dir/URL/glob, 0 for webcam')
