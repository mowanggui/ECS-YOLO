...
class DDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        self.cv2 = nn.ModuleList(
            nn.Sequential(Conv_B(x, c2, 3), Conv_B(c2, c2, 3, g=4), Snn_Conv2d(c2, 4 * self.reg_max, 1, groups=4)) for x
            in
            ch)
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv_B(x, c3, 3), Conv_B(c3, c3, 3), Snn_Conv2d(c3, self.nc, 1)) for x in ch)
        self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
...
class DualDDetect(nn.Module):
    ...
    def __init__(self, nc=80, ch=(), inplace=True):  # detection layer
        ...
        self.cv2 = nn.ModuleList(
            ...
            ch[:self.nl])
        self.cv3 = nn.ModuleList(
            nn.Sequential(Conv_B(x, c3, 3), Conv_B(c3, c3, 3), Snn_Conv2d(c3, self.nc, 1)) for x in ch[:self.nl])
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv_B(x, c4, 3), Conv_B(c4, c4, 3, g=4), Snn_Conv2d(c4, 4 * self.reg_max, 1, groups=4)) for x
            in
            ch[self.nl:])
        self.cv5 = nn.ModuleList(
            nn.Sequential(Conv_B(x, c5, 3), Conv_B(c5, c5, 3), Snn_Conv2d(c5, self.nc, 1)) for x in ch[self.nl:])
        self.dfl = DFL(self.reg_max)
        self.dfl2 = DFL(self.reg_max)
    ...
    def forward(self, x):
        ...
        d2 = []
        for i in range(self.nl):
            d1.append(torch.cat(((self.cv2[i](x[i])).sum(dim=0) / (self.cv2[i](x[i])).size()[0],
                                 (self.cv3[i](x[i])).sum(dim=0) / (self.cv3[i](x[i])).size()[0]),
                                1))
            d2.append(torch.cat(((self.cv4[i](x[self.nl + i])).sum(dim=0) / (self.cv4[i](x[self.nl + i])).size()[0],
                                 (self.cv5[i](x[self.nl + i])).sum(dim=0) / (self.cv5[i](x[self.nl + i])).size()[0]),
                                1))
        if self.training:
            return [d1, d2]
        elif self.dynamic or self.shape != shape:
        ...
        box, cls = torch.cat([di.view(shape[1], self.no, -1) for di in d1], 2).split((self.reg_max * 4, self.nc), 1)
        dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        box2, cls2 = torch.cat([di.view(shape[1], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc), 1)
        dbox2 = dist2bbox(self.dfl2(box2), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        y = [torch.cat((dbox, cls.sigmoid()), 1), torch.cat((dbox2, cls2.sigmoid()), 1)]
        return y if self.export else (y, [d1, d2])
...
class v10Detect(DDetect):
    max_det = 300

    def __init__(self, nc=80, ch=()):
        super().__init__(nc, ch)
        c3 = max(ch[0], min(self.nc, 100))  # channels
        self.cv3 = nn.ModuleList(nn.Sequential(nn.Sequential(Conv_B(x, x, 3, g=x), Conv_B(x, c3, 1)),
                                               nn.Sequential(Conv_B(c3, c3, 3, g=c3), Conv_B(c3, c3, 1)),
                                               Snn_Conv2d(c3, self.nc, 1)) for i, x in enumerate(ch))
    ...
    def forward(self, x):
        ...
        # for i in range(self.nl):
        #     one2one[i] = torch.cat(((self.one2one_cv2[i](one2one[i])).sum(dim=0) / (self.one2one_cv2[i](one2one[i])).size()[0],
        #                             (self.one2one_cv3[i](one2one[i])).sum(dim=0) / (self.one2one_cv3[i](one2one[i])).size()[0]), 1)
        one2one = [
            torch.cat(((self.one2one_cv2[i](x_detach[i])).sum(dim=0) / (self.one2one_cv2[i](x_detach[i])).size()[0],
                       (self.one2one_cv3[i](x_detach[i])).sum(dim=0) / (self.one2one_cv3[i](x_detach[i])).size()[0]), 1)
            for i in range(self.nl)
        ]
        # z = []
        # print(type(x_detach))
        # for i in range(self.nl):
        #     z[i] = torch.cat(
        #         ((self.one2one_cv2[i](x_detach[i])).sum(dim=0) / (self.one2one_cv2[i](x_detach[i])).size()[0],
        #          (self.one2one_cv3[i](x_detach[i])).sum(dim=0) / (self.one2one_cv3[i](x_detach[i])).size()[0]), 1)
        # one2one = z
        # print("a")
        # print(type(one2one))
        ...
        if not self.training:
            if self.dynamic or self.shape != shape:
                self.anchors, self.strides = (one2one.transpose(0, 1) for one2one in
                                              make_anchors(one2one, self.stride, 0.5))
                self.shape = shape

            box, cls = torch.cat([one2onei.view(shape[1], self.no, -1) for one2onei in one2one], 2).split(
                (self.reg_max * 4, self.nc), 1)
            dbox = dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
            y = torch.cat((dbox, cls.sigmoid()), 1)
            one2one = y if self.export else (y, one2one)
            # print("c")
            # print(type(one2one))
            if not self.export:
...
class ASFF_DDetect(DDetect):
    def __init__(self, nc=80, ch=()):
        super().__init__(nc, ch)
        self.nl = len(ch)
        self.asffs = nn.ModuleList(ASFF2(i) for i in range(self.nl))
...
class ASFF_v10Detect(v10Detect):
    def __init__(self, nc=80, ch=()):
        super().__init__(nc, ch)
        self.nl = len(ch)
        self.asffs = nn.ModuleList(ASFF2(i) for i in range(self.nl))
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment,
                          ASFF_DDetect, v10Detect, ASFF_v10Detect)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg='yolo.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, DDetect, Segment, v10Detect, ASFF_DDetect, ASFF_v10Detect)):
            s = 256  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment)) else self.forward(x)
            if isinstance(m, (v10Detect, ASFF_v10Detect)):
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost, Conv_2, snn_resnet, Concat_res3, Concat_res4,
                 BasicBlock, BasicBlock_1, BasicBlock_2, BasicBlock_3, Conv_A, CSABlock, LIAFBlock, Conv_LIAF,
                 Bottleneck_2, Conv_3, Conv_4, C2f, StarBlock, StarBlock_2, StarBlock_3, StarBlock_4,
                 ...
                 BasicBlock_1n, BasicBlock_1m, BasicBlock_4, Concat_res5, AIFI, RepC3, BasicELAN4, BasicBlock_5,
                 Concat_res6, MobileNetV3, BasicBlock_6, BasicBlock_1s, StarBlock_2C2f]:
            c1, c2 = ch[f], args[0]
        ...
        #     c2 = ch[f] * 4
            ...
            c2 = ch[f[-1]]
        # TODO: channel, gw, gd
        elif m in {Detect, DualDetect, TripleDetect, DDetect, DualDDetect, TripleDDetect, Segment, v10Detect,
                   ASFF_DDetect, ASFF_v10Detect}:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='res10v10.yaml', help='model.yaml')
