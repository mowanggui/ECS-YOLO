...
try:
    if RANK not in [0, -1]:
        ...
        from utils.loggerss.comet import CometLogger
...
class Loggers():
    ...
    def on_train_batch_end(self, model, ni, imgs, targets, paths, vals):
        if len(vals) == 3:
            ...
            log_dict = dict(zip(self.keys[0:3] * 2, vals))
    ...
    def on_fit_epoch_end(self, vals, epoch, best_fitness, fi, index=1):
        # Callback runs at the end of each fit (train+val) epoch
        if index == 10:
            x = dict(zip(self.keys2, vals))
        else:
        ...
        if self.csv:
            ...
            if index == 10:
                s = '' if file.exists() else (
                            ('%20s,' * n % tuple(['epoch'] + self.keys2)).rstrip(',') + '\n')  # add header
            else:
                s = '' if file.exists() else (
