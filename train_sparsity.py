...
from models.common import batch_norm_2d,batch_norm_2d1
...
def train(hyp,  # 超参数 可以是超参数配置文件的路径或超参数字典 path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            if opt.st:
                ...
                for k, m in model.named_modules():
                    if isinstance(m, batch_norm_2d) and (k not in ignore_bn_list):
                        m.bn.weight.grad.data.add_(srtmp * torch.sign(m.bn.weight.data))  # L1
                        m.bn.bias.grad.data.add_(opt.sr * 10 * torch.sign(m.bn.bias.data))  # L1
                    if isinstance(m, batch_norm_2d1) and (k not in ignore_bn_list):
                        m.bn.weight.grad.data.add_(srtmp * torch.sign(m.bn.weight.data))  # L1
                        m.bn.bias.grad.data.add_(opt.sr * 10 * torch.sign(m.bn.bias.data))  # L1
            # # ============================= sparsity training ========================== #
        ...
        #     if isinstance(layer, batch_norm_2d) and i not in ignore_bn_list:
        #         bnw = layer.bn.state_dict()['weight']
        #         bnb = layer.bn.state_dict()['bias']
        #         module_list.append(bnw)
        #         module_bias_list.append(bnb)
        #         # bnw = bnw.sort()
        #         # print(f"{i} : {bnw} : ")
        #     if isinstance(layer, batch_norm_2d1) and i not in ignore_bn_list:
        #         bnw = layer.bn.state_dict()['weight']
        #         bnb = layer.bn.state_dict()['bias']
        #         module_list.append(bnw)
        #         module_bias_list.append(bnb)
        ...
        # callbacks.add_histogram('bn_weights/hist', bn_weights.numpy(), epoch, bins='doane')
        # callbacks.add_histogram('bn_bias/hist', bnb_weights.numpy(), epoch, bins='doane')
